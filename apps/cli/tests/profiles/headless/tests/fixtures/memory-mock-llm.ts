...
class MemorySnapshotAdapter extends LlmAdapter {
  ...
  async *stream(options: GenerateOptions): AsyncIterable<StreamChunk> {
    ...
    if (toolResult === undefined) {
      ...
      yield { type: 'tool-call-delta', index: 0, id: ToolCallId('memory-snapshot-call'), name: 'memory_search', argumentsDelta: argumentsJson }
      yield { type: 'block-end', index: 0, block: { type: 'tool-call', id: ToolCallId('memory-snapshot-call'), name: 'memory_search', arguments: argumentsJson } }
