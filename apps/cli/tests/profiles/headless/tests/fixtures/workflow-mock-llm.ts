...
class WorkflowSnapshotAdapter extends LlmAdapter {
  ...
  async *stream(options: GenerateOptions): AsyncIterable<StreamChunk> {
    ...
    if (request === 0) {
      ...
      yield { type: 'tool-call-delta', index: 0, id: ToolCallId('workflow-snapshot-call'), name: 'workflow', argumentsDelta: argumentsJson }
      yield { type: 'block-end', index: 0, block: { type: 'tool-call', id: ToolCallId('workflow-snapshot-call'), name: 'workflow', arguments: argumentsJson } }
