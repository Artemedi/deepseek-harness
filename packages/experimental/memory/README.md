...
- **Opt-in HTTP routes** — TencentDB and OpenViking HTTP providers are selected only by explicit provider configuration; the default DSH/Web composition remains unchanged. OpenViking retains a deterministic local stub for contract tests, while TencentDB requires a running Gateway. DSH does not yet install or start the standalone MemoryCore runtime.
...
- **Live deployment** — TencentDB's current v3 request and response envelope is covered by mocked-fetch tests, but this checkout has no verified live Gateway. Live smoke testing remains an operator task.
