...
The opt-in TencentDB provider uses the upstream v3 data-plane contract. Deployment configuration supplies exact bindings from an absolute DSH workspace and its effective durable agent preset to provisioned TencentDB Team, Agent, and User identifiers. Omission of the preset matches only sessions composed without one. Duplicate DSH scopes, reuse of one TencentDB Team/Agent profile across scopes, and unbound callers are rejected before provider HTTP. This preserves the workspace authorization boundary through L1-L3 without inventing Team or authenticated User identity that DSH does not own. Remote provider URLs must be bare HTTP(S) origins without userinfo, path, query, or fragment. A numeric loopback standalone Gateway may use its upstream default with authentication disabled; DSH still supplies a non-secret bearer marker because the v3 data-plane parser requires the header shape. Hostname aliases and non-loopback endpoints require a non-empty DSH credential reference whose resolved secret is also non-empty. Missing connection, authentication, or isolation configuration rejects the composition instead of selecting test data. Remote response bodies are streamed into a fixed byte budget and cancelled as soon as they exceed it, including when `Content-Length` is absent.
...
L3 generation runs file tools against an isolated draft workspace. The live `persona.md` is published only after the complete runner call succeeds, the draft exists, and its content passes sanitization; a failed continuation discards the draft, leaves the prior profile unchanged, rejects the generation attempt, and therefore cannot advance the L3 checkpoint. Existing-profile and changed-scene read failures also reject instead of being interpreted as a cold start or skipped evidence. Filesystem publication uses a same-directory temporary file and rename, while storage-backed profiles use one whole-object replacement.

Optional automatic recall runs once before the first step, derives its query only from direct user input, and records the exact combined result before returning a separate model-visible reference message. It defaults to L1; deployments may opt into L2 and L3, which share one aggregate hit and byte budget. L2 uses the upstream scenario listing, ranks bounded path and summary metadata by literal query matches, and reads only selected profiles; L3 reads the singleton core profile. The message labels remote memory as potentially stale and non-instructional. A layer failure records only a code from the closed memory diagnostic vocabulary, preserves successful layers, and does not block the model turn. Cancellation still propagates instead of becoming fail-open.
...
**Use one static TencentDB isolation tuple for the deployment.** Rejected because L3 is a Team/Agent profile: unrelated workspaces or agent presets would read and update the same core memory even when their DSH session histories are isolated.

**Let L3 file tools write the live profile and roll back after failure.** Rejected because readers can observe the intermediate write and rollback itself can fail. Drafting outside the live profile gives publication one explicit commit point.
...
- Every TencentDB request resolves an exact workspace/effective-preset binding; ambiguous configuration and unbound callers fail before provider HTTP.
- A failed L3 runner continuation cannot change the live persona after an earlier tool write in the same run.
- Automatic recall preserves successful layers during an outage and records only a bounded, non-sensitive diagnostic code.
- Generated persistence vocabulary includes every durable memory event so capture and recall logs remain resumable.
- Remote endpoints reject embedded URL state, and response bodies cannot be buffered beyond their configured byte limit.

## Consequences

The local provider searches only session history. TencentDB L2 does not expose a semantic search endpoint, so DSH matches only bounded scenario path and summary metadata before selected reads; it never scans every scenario body. Workspace moves and preset renames require binding updates; DSH does not migrate provider profiles. Capture remains at least once until upstream accepts an idempotency key or stable client message identifier with atomic uniqueness. All remote layers require provider-specific authorization, response bounds, failure behavior, and durable event semantics before model use.
